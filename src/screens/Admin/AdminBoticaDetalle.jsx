...
// ── Pantalla principal ─────────────────────────────────────────────
export default function AdminBoticaDetalle() {
  const { boticaId } = useParams()
  ...
  const fetchConsultas = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select(`
        ...
        doctor:doctors!doctor_id(especialidad)
      `)
      .eq('farmacia_referente_id', boticaId)
      .eq('status', 'done')
  ...
  const totalMonto     = totalPacientes * COMISION_BOTICA
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.gray100, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <header style={{
        ...
        <button
          onClick={() => navigate('/admin/boticas')}
          style={{
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 22, fontWeight: 900, color: C.gray900, lineHeight: 1.2 }}>
              {botica?.nombre ?? <span style={{ color: C.gray300 }}>Cargando…</span>}
            </div>
        ...
        <div style={{
          ...
          <div style={{ padding: '16px 20px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h2 style={{ fontSize: 14, fontWeight: 800, color: C.gray900, margin: 0 }}>
              Consultas referidas — {MESES_FULL[mes]} {anio}
            </h2>
            {!loading && (
              <span style={{
                  ...
                  ? (
                    <tr>
                      <td colSpan={4} style={{ padding: '40px 16px', textAlign: 'center', color: C.gray400, fontSize: 13 }}>
                        Sin pacientes referidos atendidos en {MESES_FULL[mes]} {anio}
                      </td>
                    </tr>
                  )
                  : consultas.map(a => {
                    const docRaw = a.doctor
              ...
              <tfoot>
                <tr>
                  <td colSpan={3} style={{
                    ...
                    Total a pagar — {totalPacientes} paciente{totalPacientes !== 1 ? 's' : ''} × {fmtSoles(COMISION_BOTICA)}:
                  </td>
                ...
                content={({ active, payload, label }) => {
                  ...
                  return (
                    <div style={{
                      ...
                      <div>{n} paciente{n !== 1 ? 's' : ''} referido{n !== 1 ? 's' : ''}</div>
                      <div style={{ marginTop: 2, opacity: 0.8 }}>{fmtSoles(n * COMISION_BOTICA)}</div>
                    </div>
                  )
                }}
                cursor={{ fill: C.green50 }}
              />
              <Bar dataKey="pacientes" fill={C.green500} radius={[5, 5, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
