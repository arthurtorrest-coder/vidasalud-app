...
function Spinner() {
  return (
    <div style={{
      ...
      animation: 'se-spin 0.75s linear infinite', flexShrink: 0,
    }} />
  )
}
        ...
        return (
          <div key={i} style={{
            ...
            background: isMe
              ...
              : iDone ? C.gray200 : C.green100,
            border: `2px solid ${isMe ? C.green500 : iDone ? C.gray200 : C.green200}`,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: isMe ? 12 : 9, fontWeight: 800,
            color: isMe ? C.white : iDone ? C.gray400 : C.green700,
            transition: 'all 0.3s',
...
export default function SalaEspera() {
  ...
  const loadData = useCallback(async (quiet = false) => {
    ...
    if (['done', 'cancelled'].includes(myAppt.status)) {
      setLoading(false)
      setRefreshing(false)
      return
    }
    ...
    const { data: colaRaw } = await supabase
      ...
      .eq('doctor_id', myAppt.doctor?.id ?? myAppt.doctor_id)
  ...
  // Realtime: cualquier UPDATE a appointments de este médico
  useEffect(() => {
    if (!appt) return
  ...
  // Cuenta regresiva + auto-refresh cada 30s
  useEffect(() => {
  ...
  const esDone    = appt?.status === 'done'
  const listoCola = !esActiva && adelante === 0 && appt?.status === 'paid'
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        @keyframes se-pulse  { 0%,100%{transform:scale(1)} 50%{transform:scale(1.12)} }
        @keyframes se-glow   { 0%,100%{box-shadow:0 0 0 0 rgba(16,185,129,0)} 50%{box-shadow:0 0 0 14px rgba(16,185,129,0.18)} }
        @keyframes se-slide  { from{opacity:0;transform:translateY(12px)} to{opacity:1;transform:none} }
        @keyframes se-blink  { 0%,100%{opacity:1} 50%{opacity:0.4} }
      `}</style>
      ...
      <div style={{
        ...
        {loading ? (
          ...
          <>
            {/* ── Estado principal ── */}
            <div style={{
              background: C.white, border: `1.5px solid ${esActiva ? C.green300 : listoCola ? C.green200 : C.gray200}`,
                ...
                {esActiva ? (
                  ...
                  <>
                    <div style={{
                      ...
                      animation: 'se-blink 1.2s step-end infinite',
                    }}>🔔</div>
                    <div style={{
                      fontSize: 22, fontWeight: 900, color: C.green800,
                    ...
                    <button
                      ...
                      style={{
                        ...
                        boxShadow: '0 6px 20px rgba(5,150,105,0.4)', fontFamily: 'inherit',
                  ...
                  <>
                    <div style={{ fontSize: 48 }}>⚡</div>
                    <div style={{
                      fontSize: 20, fontWeight: 900, color: C.green800, marginTop: 10,
                    }}>
                      ¡Eres el siguiente!
                    </div>
                    <div style={{ fontSize: 13, color: C.green700, marginTop: 6, lineHeight: 1.5 }}>
                  ...
                  <>
                    ...
                    <div style={{
                      ...
                      background: C.gray100, borderRadius: 20,
            ...
            {/* ── Tiempo estimado (solo si hay espera) ── */}
            {!esActiva && !listoCola && (
              <div style={{
            ...
            {/* ── Nota privacidad ── */}
            <div style={{
              textAlign: 'center', fontSize: 10, color: C.gray400, lineHeight: 1.5,
            }}>
